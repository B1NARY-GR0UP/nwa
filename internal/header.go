...
var (
	ErrUnsupportedFileType = errors.New("file type not supported, header generated failed")
	ErrInvalidStyleString  = errors.New("invalid style string")
)
...
func generateHeader(path string, tmpl []byte, styles []string) ([]byte, error) {
	var header []byte
	...
	// return cache if the header is generated by the file with the same extension
	if h, ok := cache[ext]; ok {
		return h, nil
	}

	styleMap, err := parseStyles(styles)
	if err != nil {
		return nil, err
	}

	for _, extMap := range extMaps {
		// check if match comment style tag (`ext:style`) first
		if style, ok := styleMap[ext]; ok && extMap.name == style {
		...
		// if not matched style, fallback to default style
...
// parse style strings in `extension:style` format
func parseStyles(styles []string) (map[string]string, error) {
	m := make(map[string]string, len(styles))
	for _, style := range styles {
		parts := strings.SplitN(style, ":", 2)
		if len(parts) != 2 || parts[0] == "" || parts[1] == "" {
			return nil, ErrInvalidStyleString
		}
		ext := parts[0]
